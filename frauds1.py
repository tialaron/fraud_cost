...
'Имеется датасет обработанных банковских транзакций. Обработка выполнена математическим способом, чтобы исключить передачу персональных данных клиентов. '
data1 = df.drop(['Time'],axis=1)
data1['Amount'] = StandardScaler().fit_transform(data1['Amount'].values.reshape(-1, 1))
st.write(data1)
'Мы берем обезличенные данные. Каждая из операций либо стандартная (честная), либо мошенническая. В колонке Class стандартные операции обозначаются как 0, а мошеннические - 1.'

frauds1 = data1[data1.Class == 1]
normal1 = data1[data1.Class == 0]

'Мошеннических операций в данных гораздо меньше чем стандартных (честных).'
